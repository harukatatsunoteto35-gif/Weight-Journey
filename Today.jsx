    ...
    <div style={{ padding: "0 24px" }}>
      ...
      <div style={cardStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: progressPct < 0 ? C.danger : C.rose }}>{Math.round(progressPct)}%</span>
        </div>
        <div style={{ width: "100%", height: 8, borderRadius: 999, background: C.cardBorder, overflow: "hidden" }}>
          <div style={{ height: "100%", borderRadius: 999, width: `${Math.max(0, Math.min(100, progressPct))}%`, background: progressPct < 0 ? C.danger : `linear-gradient(to right, ${C.roseSoft}, ${C.rose})` }} />
        </div>
